...
public class FixtureJoiner {
    ...
    public FixtureJoiner(int startingSeasonStart, int startingSeasonEnd, int endingSeasonEnd) throws IllegalArgumentException {
    ...
    public void joinFixtureData(FileWriter fileWriter, String baseFilePath, String subFilePath) {
        ...
        try {
            for (int i = this.startingSeasonStart, j = this.startingSeasonEnd; j <= this.endingSeasonEnd; i++, j++) {
                File folder = new File(String.format("%s%s-%s/fixtures/", baseFilePath, i, j));
            ...
            LOGGER.error("Error joining fixture files together: " + ioException.getMessage());
        }
    }
