...
public class GameweekJoinerTest extends TestWriterHelper {
    ...
    private static final String SEASON_FILEPATH = "src/test/resources/data/2022-23/";
    private static final String MERGED_FILENAME = "merged_gw.csv";
    private static final String FULL_FILEPATH = String.format("%s%s/%s", SEASON_FILEPATH, SEASON, MERGED_FILENAME);
    private static final FileWriter FILE_WRITER = new FileWriter(SEASON_FILEPATH, "2022-23");
    ...
    public void joinGameweeks() {
        GAMEWEEK_JOINER = new GameweekJoiner(CURRENT_GAMEWEEK_NUMBER, seasonFilePath, subFilePath, fileWriter);
        GAMEWEEK_JOINER.join(FILE_WRITER, SEASON_FILEPATH, MERGED_FILENAME);
    ...
    public void invalidCurrentGameweekNumberZero() {
        GAMEWEEK_JOINER = new GameweekJoiner(0, seasonFilePath, subFilePath, fileWriter);
    }

    @Test(expected = IllegalArgumentException.class)
    public void invalidCurrentGameweekNumberLessThanZero() {
        GAMEWEEK_JOINER = new GameweekJoiner(-1, seasonFilePath, subFilePath, fileWriter);
    }
}
